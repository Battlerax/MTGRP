...
namespace mtgvrp.job_manager.garbageman
{
    public class Garbageman : Script
    {
        ...
        public void GarbageThrowBag(Player player, params object[] arguments)
        {
            ...
            if (player.Rotation.Z > NAPI.Entity.GetEntityRotation(closestVeh.Entity).Z + 30 || player.Rotation.Z < NAPI.Entity.GetEntityRotation(closestVeh.Entity).Z - 30)
            {
        ...
        public void OnPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seat)
        {
            ...
            if (veh?.Job?.Type == JobManager.JobTypes.Garbageman && character.JobOne?.Type == JobManager.JobTypes.Garbageman)
            {
                ...
                {
                    NAPI.ClientEvent.TriggerClientEvent(player, "garbage_setwaypoint", character.JobOne.MiscOne.Location);
                    player.SendChatMessage("You've got 10 bags of garbage in this truck. Unload it at the waypoint on your map.");
                }
                else
                {
                    NAPI.ClientEvent.TriggerClientEvent(player, "garbage_setwaypoint", targetProperty.GarbagePoint);
        ...
        public void unloadtrash_cmd(Player player)
        {
            ...
            if (player.Rotation.Z > NAPI.Entity.GetEntityRotation(closestVeh.Entity).Z + 20 || player.Rotation.Z < NAPI.Entity.GetEntityRotation(closestVeh.Entity).Z - 20)
            {
                player.SendChatMessage("~r~You must be at the back of your garbage truck to unload the trash.");
                return;
            }
        ...
        public void pickuptrash_cmd(Player player)
        {
            ...
            if (prop == null)
            {
                NAPI.Chat.SendChatMessageToPlayer(player, "You aren't at a garbage point.");
                return;
            }
